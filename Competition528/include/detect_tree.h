#ifndef DETECT_TREE_H
#define DETECT_TREE_H
...
double TreeCalculateTheta(Vec2d &oc, Vec2d &pt_0, Vec2d &pt_x);
bool TreePointCmp(Vec2d &a, Vec2d &b, Vec2d &center);

void TreeImageThreshold(Mat imgOriginal, Mat &imgThresholded)
{
...
inline double TreeCalculateTheta(Vec2d &oc, Vec2d &pt_0, Vec2d &pt_x)
{
	...
	if (TreePointCmp(pt_0, pt_x, oc))
		return 6.28 - acos(cos_theta);
	else
		return acos(cos_theta);
}

inline bool TreePointCmp(Vec2d &a, Vec2d &b, Vec2d &center)
{
...
void  TreeTwo_PassNew(const Mat &bwImg, Mat &labImg)
{
...
std::vector<Vec2i> detect_tree(cv::Mat oriImage, float height)
{
	std::vector<Vec2i> coordination;
	cv::Mat one_image;
	cv::Mat binImage;
	TreeImageThreshold(oriImage, binImage);
	cv::threshold(binImage, binImage, 50, 1, CV_THRESH_BINARY);
	cv::Mat labelImg;
	double time;
	time = getTickCount();
	
	TreeTwo_PassNew(binImage, labelImg);
	...
	while (1)
	{
		...
		if ((x_max - x_min) <Min_pixel_num || (y_max - y_min) <Min_pixel_num)
		{
			x_min = labelImg.rows;
			y_min = labelImg.cols;
			x_max = 0;
