#ifndef DETECT_NUM_NAV_H
#define DETECT_NUM_NAV_H
...
double CalculateThetaNav(Vec2d &oc, Vec2d &pt_0, Vec2d &pt_x);
bool PointCmpNav(Vec2d &a, Vec2d &b, Vec2d &center);
void ClockSortPointsNav(std::vector<Vec2d> &vPoints, Vec2d &center, Vec2d &pt_0);

void FindMinMaxNav(int &x_min, int &x_max, int &y_min, int &y_max, Mat image)
{
	x_max = 0;
	y_max = 0;
	x_min = image.rows;
...
static void ImageThresholdNav(Mat imgOriginal, Mat &imgThresholded)
{
	...
	imwrite("ThresholdedNum.jpg", imgThresholded);
...
inline double CalculateThetaNav(Vec2d &oc, Vec2d &pt_0, Vec2d &pt_x)
{
	...
	if (PointCmpNav(pt_0, pt_x, oc))
		return 6.28 - acos(cos_theta);
	else
		return acos(cos_theta);
}

inline bool PointCmpNav(Vec2d &a, Vec2d &b, Vec2d &center)
{
...
inline  void ClockSortPointsNav(std::vector<Vec2d> &vPoints, Vec2d &center, Vec2d &pt_0)
{
	for (int i = 0; i < vPoints.size() - 1; i++)
	{
		for (int j = 0; j < vPoints.size() - i - 1; j++)
		{
			if (CalculateThetaNav(center, pt_0, vPoints[j])>CalculateThetaNav(center, pt_0, vPoints[j + 1]))
			{
				Vec2d tmp = vPoints[j];
...
void  Two_PassNewNav(const Mat &bwImg, Mat &labImg)
{
...
std::vector<Vec3i> detect_num_nav(cv::Mat oriImage, std::vector<cv::Mat> &all_image)
{
	std::vector<Vec3i> coordination;
	cv::Mat one_image;
	cv::Mat binImage;
	ImageThresholdNav(oriImage, binImage);
	cv::threshold(binImage, binImage, 50, 1, CV_THRESH_BINARY);
	cv::Mat labelImg;
	double time;
	time = getTickCount();
	
	Two_PassNewNav(binImage, labelImg);
	...
	while (1)
	{
		...
		if (!t)
			break;
		if ((x_max - x_min) < 18 || (y_max - y_min) < 18 || (y_max - y_min) > 200)
		{
			x_min = labelImg.rows;
			y_min = labelImg.cols;
			x_max = 0;
